...
import { PrayerSyncService } from '../../src/modules/prayer/prayer.sync.service';
import { PrismaService } from '../../src/database/prisma.service';
import { CommonHttpService } from '../../src/common/common.module';
import { PrayerMapper } from '../../src/modules/prayer/prayer.mapper';
...
describe('PrayerSyncService', () => {
  ...
  describe('idempotency', () => {
    it('should handle duplicate records gracefully', async () => {
      ...
      // First call succeeds
      (prismaService as any).prayerTimes.create.mockResolvedValueOnce({});
      
      // Second call fails with unique constraint error, then succeeds with update
      const uniqueError = new Error('Unique constraint failed');
      (uniqueError as any).code = 'P2002';
      (prismaService as any).prayerTimes.create.mockRejectedValueOnce(uniqueError);
      (prismaService as any).prayerTimes.update.mockResolvedValueOnce({});
