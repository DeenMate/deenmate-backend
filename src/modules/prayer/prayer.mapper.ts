...
export class PrayerMapper {
  mapCalculationMethodFromUpstream(upstream: UpstreamCalculationMethod) {
    const fajr =
      typeof upstream.params?.Fajr === "number" ? upstream.params.Fajr : 18;
    ...
    return {
      ...
      methodCode: this.generateMethodCode(
        upstream.name || `Method_${upstream.id}`,
      ),
  ...
  mapPrayerTimesFromUpstream(
    ...
    return {
      ...
      midnight: timings.Midnight
        ? this.parseTimeString(timings.Midnight, date)
        : null,
  ...
  mapPrayerTimesToUpstreamFormat(times: any) {
    ...
    return {
      timings: {
        ...
        Imsak: times.imsak
          ? this.formatTimeForUpstream(times.imsak)
          : undefined,
        Midnight: times.midnight
          ? this.formatTimeForUpstream(times.midnight)
          : undefined,
      },
      date: {
