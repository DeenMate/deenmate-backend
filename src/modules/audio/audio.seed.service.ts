  ...
  async seedReciters(): Promise<void> {
    ...
    for (const reciter of reciters) {
      try {
        ...
        this.logger.error(
          `Failed to seed reciter ${reciter.name}: ${error.message}`,
        );
      }
    }
  ...
  async seedAudioFiles(): Promise<void> {
    ...
    for (const reciter of reciters) {
      for (const verse of verses) {
        try {
          const audioUrl = this.generateAudioUrl(
            reciter.sourceId,
            verse.chapterNumber,
            verse.verseNumber,
          );
          ...
          this.logger.error(
            `Failed to seed audio file for verse ${verse.chapterNumber}:${verse.verseNumber}: ${error.message}`,
          );
        }
      }
    }
    ...
    this.logger.log(
      `Audio files seed completed. Processed ${audioFilesProcessed} audio files.`,
    );
  }
