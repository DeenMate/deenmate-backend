...
describe('QuranController', () => {
  ...
  describe('getChapterVerses', () => {
    it('should return verses for a chapter', async () => {
      ...
      const result = await controller.getChapterVerses('1', 'en', '', 1, 10, mockRes as any);
  ...
  describe('getTranslations', () => {
    it('should return translation resources', async () => {
      ...
      const result = await controller.getTranslations('en', mockRes as any);
      
      expect(result).toBeDefined();
      expect(service.getTranslationResourcesFiltered).toHaveBeenCalledWith('en', 1, 50);
    });
  });
});
