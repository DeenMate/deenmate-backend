...
export class QuranService {
  ...
  async getChapter(chapterNumber: number) {
    try {
      ...
      if (!chapter) {
        this.logger.warn(`Chapter ${chapterNumber} not found in database, falling back to upstream API`);
      ...
      this.logger.error(`Failed to get chapter ${chapterNumber} from database: ${error.message}`);
      return await this.fallbackToUpstream('chapter', { chapterNumber });
    }
  }
  ...
  async getVerse(verseKey: string) {
    try {
      ...
      if (!verse) {
        this.logger.warn(`Verse ${verseKey} not found in database, falling back to upstream API`);
      ...
      this.logger.error(`Failed to get verse ${verseKey} from database: ${error.message}`);
      return await this.fallbackToUpstream('verse', { verseKey });
    }
  }
  ...
  async getTranslationResources(page = 1, limit = 10) {
    try {
      ...
      if (resources.length === 0) {
        this.logger.warn('No translation resources found in database, falling back to upstream API');
      ...
      this.logger.error(`Failed to get translation resources from database: ${error.message}`);
      return await this.fallbackToUpstream('translations', { page, limit });
    }
  }

  // New: List translation resources with optional language filter (DB-first, fallback upstream)
  async getTranslationResourcesFiltered(language?: string, page = 1, limit = 50) {
    try {
      ...
      if (resources.length === 0) {
        return await this.fallbackToUpstream('translations', { language, page, limit });
      }
      return {
        code: 200,
        status: 'Success',
      ...
      this.logger.error(`Failed to list translation resources (filtered): ${e instanceof Error ? e.message : String(e)}`);
      return await this.fallbackToUpstream('translations', { language, page, limit });
    }
  }
  ...
  async getVerseTranslations(verseKey: string, resourceIds?: number[]) {
    try {
      const verse = await (this.prisma as any).quranVerse.findUnique({
        ...
        include: {
          translations: {
            where: resourceIds && resourceIds.length > 0 ? { resourceId: { in: resourceIds } } : {},
            include: {
              resource: {
                select: {
                  name: true,
      ...
      const keySuffix = resourceIds && resourceIds.length > 0 ? `rids_${resourceIds.sort((a,b)=>a-b).join('_')}` : 'auto_enbn';
      ...
      if (!verse || verse.translations.length === 0) {
        ...
        if (resourceIds && resourceIds.length > 0) {
          const res = await this.fallbackToUpstream('verse-translations', { verseKey, resourceIds });
          if (res.code === 200) {
        ...
        if (!autoIds || autoIds.length === 0) {
          return {
            code: 400,
            status: 'Bad Request',
            message: 'resource_ids are required when translation resources cannot be resolved upstream',
          } as any;
        }
        this.logger.warn(`No translations found for verse ${verseKey} in DB, falling back with EN/BN IDs: ${autoIds.join(',')}`);
        const res = await this.fallbackToUpstream('verse-translations', { verseKey, resourceIds: autoIds });
        if (res.code === 200) {
          await this.redis.set(cacheKey, JSON.stringify(res.data), 21600);
          if (res.headers) res.headers['X-DeenMate-Cache'] = 'miss';
        }
      ...
      this.logger.error(`Failed to get translations for verse ${verseKey} from database: ${error.message}`);
      // On error, if explicit IDs exist use them; otherwise try EN/BN, else 400
      if (resourceIds && resourceIds.length > 0) {
        return await this.fallbackToUpstream('verse-translations', { verseKey, resourceIds });
      }
      const autoIds = await this.resolveEnBnResourceIds();
      if (!autoIds || autoIds.length === 0) {
        return {
      ...
      return await this.fallbackToUpstream('verse-translations', { verseKey, resourceIds: autoIds });
    }
  }
  ...
  async searchQuran(query: string, page = 1, limit = 10) {
    try {
      ...
      if (verses.length === 0) {
        this.logger.warn(`No search results found for "${query}" in database, falling back to upstream API`);
  ...
  async getReciters(language?: string) {
    ...
    if (cached) {
      ...
      return { code: 200, status: 'Success (Cache)', data, headers: { 'X-DeenMate-Source': 'cache', 'X-DeenMate-Cache': 'hit' } };
    }
  ...
  async getRecitationByChapter(recitationId: number, chapterNumber: number, page?: number, perPage?: number) {
    ...
    const res = await this.fallbackToUpstream('recitation-by-chapter', { recitationId, chapterNumber, page, perPage });
    if (res.code === 200) {
      await this.redis.set(cacheKey, JSON.stringify(res.data), 21600);
      if (res.headers) res.headers['X-DeenMate-Cache'] = 'miss';
    }
  ...
  async getRecitationByAyah(recitationId: number, verseKey: string) {
    ...
    const res = await this.fallbackToUpstream('recitation-by-ayah', { recitationId, verseKey });
    if (res.code === 200) {
      await this.redis.set(cacheKey, JSON.stringify(res.data), 21600);
      if (res.headers) res.headers['X-DeenMate-Cache'] = 'miss';
    }
  ...
  async getVerseTafsir(verseKey: string, tafsirId?: number) {
    const cacheKey = `quran:tafsir:${verseKey}:${tafsirId || 'all'}:v1`;
    const cached = await this.redis.get(cacheKey);
    if (cached) {
      const data = JSON.parse(cached);
      return { code: 200, status: 'Success (Cache)', data, headers: { 'X-DeenMate-Source': 'cache', 'X-DeenMate-Cache': 'hit' } };
    }
    const res = await this.fallbackToUpstream('verse-tafsir', { verseKey, tafsirId });
    if (res.code === 200) {
      await this.redis.set(cacheKey, JSON.stringify(res.data), 21600);
      if (res.headers) res.headers['X-DeenMate-Cache'] = 'miss';
    }
  ...
  private async resolveEnBnResourceIds(): Promise<number[]> {
    ...
    try {
      const envIdsRaw = process.env.QURAN_FALLBACK_TRANSLATION_IDS || process.env.QURAN_FALLBACK_TRANSLATION_IDS_EN_BN;
      if (envIdsRaw) {
        ...
        if (envIds.length > 0) {
          ...
          this.logger.warn(`Using env-based fallback translation IDs: ${envIds.join(',')}`);
          return envIds;
        }
      }
    } catch (e) {
    ...
    try {
      const body = await this.httpService.get<any>(`${this.upstreamBaseUrl}/resources/translations`, { timeout: 15000 });
      const translations: any[] = body?.translations || [];
      const ids: number[] = [];
      for (const t of translations) {
        const lang = (t.language || t.language_name || '').toLowerCase();
        if (lang === 'en' || lang === 'english' || lang === 'bn' || lang === 'bangla' || lang === 'bengali') {
      ...
      this.logger.error(`Failed to resolve EN/BN resource IDs: ${e instanceof Error ? e.message : String(e)}`);
      return [];
    }
  }
    ...
    try {
      ...
      switch (endpoint) {
        ...
        case 'chapter':
          ...
          break;
        case 'verses':
          url += `/verses/by_chapter/${params.chapterNumber}?page=${params.page}&per_page=${params.limit}`;
          break;
        case 'verse':
        ...
        case 'verses-with-translations': {
          const qp: string[] = [];
          if (params.language) qp.push(`language=${encodeURIComponent(params.language)}`);
          if (Array.isArray(params.translationIds) && params.translationIds.length) qp.push(`translations=${params.translationIds.join(',')}`);
        ...
        case 'verse-translations':
          url += `/verses/by_key/${params.verseKey}/translations`;
          if (Array.isArray(params.resourceIds) && params.resourceIds.length > 0) {
            url += `?resource_ids=${params.resourceIds.join(',')}`;
          } else if (params.resourceId) {
            url += `?resource_ids=${params.resourceId}`;
          }
          break;
        case 'tafsirs':
          ...
          break;
        case 'verse-tafsir':
          url += `/verses/by_key/${params.verseKey}/tafsirs`;
          if (params.tafsirId) url += `?tafsir_ids=${params.tafsirId}`;
          break;
        case 'reciters':
