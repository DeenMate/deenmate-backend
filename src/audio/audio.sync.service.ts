...
export class AudioSyncService {
        ...
        try {
          ...
          if (existingReciter) {
            ...
            await this.prisma.quranReciter.update({
              ...
              data: {
                name: reciter.name,
                englishName: reciter.english_name || reciter.name,
                languageName: reciter.language_name || 'arabic',
                isActive: true,
              },
            });
          } else {
  ...
  async syncAudioFilesForChapter(
    ...
    try {
      this.logger.log(`Syncing audio files for reciter ${reciterId}, chapter ${chapterId}`);

      // Fetch audio files from upstream API
      const response = await firstValueFrom(
        this.httpService.get(`${this.quranApiBase}/recitations/${reciterId}/by_chapter/${chapterId}`)
      );

      const audioFiles = response.data.audio_files || [];
      this.logger.log(`Found ${audioFiles.length} audio files for chapter ${chapterId}`);
      ...
      for (const audioFile of audioFiles) {
        try {
          ...
          const verse = verses.find(v => v.verseNumber === parseInt(audioFile.verse_key.split(':')[1]));
          if (!verse) {
            this.logger.warn(`Verse not found for audio file: ${audioFile.verse_key}`);
            continue;
          }
      ...
      this.logger.log(`Successfully synced ${audioFilesProcessed} audio files for chapter ${chapterId}`);
    ...
    try {
      ...
      for (const reciter of reciters) {
        for (let chapterId = 1; chapterId <= 114; chapterId++) {
          try {
            const result = await this.syncAudioFilesForChapter(reciter.id, chapterId);
    ...
    try {
      await this.prisma.syncJobLog.create({
        data: {
          ...
          recordsProcessed: metadata.recitersProcessed || metadata.audioFilesProcessed || 0,
