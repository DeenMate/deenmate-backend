...
export class SyncCronService {
  ...
  private async syncMajorCitiesPrayerTimes() {
    ...
    for (const city of majorCities) {
      try {
        ...
        this.logger.log(`Prayer times sync for ${city.name}: ${JSON.stringify(result)}`);
  ...
  async manualQuranSync(force = false) {
    ...
    try {
      const chaptersResult = await this.quranSyncService.syncChapters({ force });
      const versesResult = await this.quranSyncService.syncVerses({ force });
      const translationsResult = await this.quranSyncService.syncTranslationResources({ force });
  ...
  async manualPrayerSync(force = false) {
    ...
    try {
      const methodsResult = await this.prayerSyncService.syncCalculationMethods({ force });
